...
public:
  ...
  void addElement(T data) {
    ...
    // 1 - Check if we are adding 1, because all other elements in the tree will definitely have 1 as a parent ( for all data, 1 | data)
    ...
    if(data == 1) {
      ...
      // If a node value can divide the data, the add a new node for the data to the tree and the node found as its parent
      // The relation in this tree is that the data in a parent of a node can divide the data in the node
      for(j = 0;j < k;j ++) {
