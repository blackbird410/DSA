...
public:
  ...
  int operator==(Grid *p) {
    return state == p->getState() && dir[UP] == p->getDir(UP) &&
           dir[DOWN] == p->getDir(DOWN) && dir[LEFT] == p->getDir(LEFT) &&
           dir[RIGHT] == p->getDir(RIGHT);
  }
...
public:
  ...
  */
  Grid *removeElement() { return (top > 0) ? data[--top] : nullptr; }
  void printPath() {
    int j;
    for (j = 1; j < top; j++) {
  ...
  void initMaze(int s) {
    ...
    for (i = 0; i < s; ++i) {
      for (j = 0; j < s; ++j) {
        ...
        if (i > 0)
          maze[index].setDir(UP, &maze[(i - 1) * s + j]);
        if (i < s - 1)
          maze[index].setDir(DOWN, &maze[(i + 1) * s + j]);
        if (j > 0)
          maze[index].setDir(LEFT, &maze[i * s + (j - 1)]);
        if (j < s - 1)
          maze[index].setDir(RIGHT, &maze[i * s + (j + 1)]);
      }
    }
